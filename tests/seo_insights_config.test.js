...
const securityCoGa4Scope = ga4DailySql.match(/WITH qualifying_sessions_Seg_CO AS \([\s\S]*?\n  \)\n  SELECT/)[0];
...
assert.ok(
  cashEsGa4Scope.includes("REGEXP_CONTAINS(LOWER(COALESCE(e.page.location, ''))") &&
    ...
    securityCoGa4Scope.includes("REGEXP_CONTAINS(LOWER(COALESCE(e.page.location, ''))") &&
    !cashEsGa4Scope.includes("event_params_custom.BusinessType") &&
    !cashPtGa4Scope.includes("event_params_custom.BusinessType") &&
    !securityCoGa4Scope.includes("event_params_custom.BusinessType"),
